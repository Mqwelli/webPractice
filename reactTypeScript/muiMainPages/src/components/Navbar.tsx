...
function Navbar({active} : Props) {
	...
	function barMenuItem(num: string, val: string, page: string) {
		return (
	...
	function barButton(num: string, val: string, page: string) {
		return (
			<Link to={page}>
				<Button 
			...
			<Container maxWidth="xl">
				<StyledToolbar>
					...
					<Box sx={{ display: { xs: 'none', md: 'flex' } }}>
						...
						{barButton('2','Solutions','/list')}
						{barButton('3','Comparisons','/chart')}
					</Box>
					<Box sx={{ display: { xs: 'flex', md: 'none' }}}>
						...
						>
							<Box>
								...
								{barMenuItem('2','Solutions','/list')}
								{barMenuItem('3','Comparisons','/chart')}
							</Box>
						</Drawer>
					</Box>
				</StyledToolbar>
